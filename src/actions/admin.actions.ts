...
async function requireAuth() {
    // Basic auth check using Supabase Admin client or a simple bypass for now
    // Since this is a server action, in Sàn Uy Tín we should rely on the client Guard or check session
    return { email: 'admin@sanuytin.net', role: 'admin', tenant_id: null as string | null, username: 'Admin' };
}

// Strip Tenant Integrity Context
...
function applySecureMatch(queryBuilder: any, match: { column: string, value: string }, forceTenantId: string | null) {
    return queryBuilder.eq(match.column, match.value);
}
...
export async function createPostAction(post: Partial<Post>, providedTenantId?: string | null) {
    try {
        ...
        // Sync category_id to string 'category' for Sàn Uy Tín frontend
        if (safeData.category_id) {
            const { data: catData } = await supabaseAdmin.from('categories').select('slug').eq('id', safeData.category_id).single();
            if (catData?.slug) {
                (safeData as any).category = catData.slug;
            }
        }
        ...
        const newPost = (data as Post[] | null)?.[0] || null;
        if (newPost) {
            let statusText = "🟢 Vừa tạo mới Bản nháp";
            if (newPost.is_published) statusText = "🚀 VỪA ĐĂNG BÀI VIẾT HOT";
            else if (newPost.scheduled_at) statusText = "⏳ Đã lên lịch tự động";
            
            const adminName = user.email || user.username || 'Admin';
            // await sendTelegramMessage(`<b>${statusText}</b>\n\n📌 <b>Tiêu đề:</b> ${newPost.title}\n✍️ <b>Tác giả:</b> ${adminName}\n🔗 <b>URL:</b> https://sanuytin.net/blog/${newPost.slug}`);
        }
        return newPost;
...
export async function updatePostAction(id: string, updates: Partial<Post>) {
    try {
        ...
        if (safeUpdates.category_id) {
            const { data: catData } = await supabaseAdmin.from('categories').select('slug').eq('id', safeUpdates.category_id).single();
            if (catData?.slug) {
                (safeUpdates as any).category = catData.slug;
            }
        }

        let query = supabaseAdmin.from('posts').update(safeUpdates);
        ...
        const updatedPost = (data as Post[] | null)?.[0] || null;
        if (updatedPost) {
            let actionText = "📝 Vừa sửa bài viết";
            if (updates.is_published === true) actionText = "🚀 VỪA XUẤT BẢN BÀI VIẾT";
            else if (updates.scheduled_at !== undefined) actionText = "⏳ Vừa thay đổi lịch đăng";
            
            const adminName = user.email || user.username || 'Admin';
            // await sendTelegramMessage(`<b>${actionText}</b>\n\n📌 <b>Tiêu đề:</b> ${updatedPost.title}\n✍️ <b>Tác giả:</b> ${adminName}\n🔗 <b>URL:</b> https://sanuytin.net/blog/${updatedPost.slug}`);
        }
        return updatedPost;
    } catch (e) { console.error(e); return null; }
}
