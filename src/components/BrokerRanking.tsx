...
function BrokerCard({ broker, index }: { broker: any, index: number }) {
    ...
    return (
        ...
        >
            {/* Rank Badge */}
            <div className={`
                absolute top-0 left-0 px-4 py-1.5 rounded-br-2xl text-sm font-bold border-r border-b border-border dark:border-border/50 z-20 transition-colors
            ...
            {index === 0 && (
                <div className="absolute top-5 -right-12 bg-accent text-accent-foreground font-bold w-40 h-8 flex items-center justify-center text-[10px] uppercase tracking-wider rotate-45 shadow-sm z-20">
                    Recommended
                </div>
            )}
            ...
            <div className="w-full lg:w-48 flex flex-col items-center gap-4 shrink-0 mt-6 lg:mt-0">
                ...
                <a href={getRelativePath(pathname, `/${broker.slug}`)} className="text-lg font-bold text-foreground hover:text-primary transition-colors text-center">
                    {broker.name}
                </a>

                <div className="flex items-center gap-2 bg-blue-50 dark:bg-primary/10 px-4 py-1.5 rounded-full border border-blue-100 dark:border-primary/20">
                    <span className="font-extrabold text-primary text-xl leading-none">{broker.score}</span>
            ...
            <div className="flex-1 w-full grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8 text-sm border-t lg:border-t-0 lg:border-l border-border dark:border-border pt-6 lg:pt-0 lg:pl-8">
                <div className="space-y-1.5 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 text-foreground dark:text-gray-200 font-bold overflow-hidden">
                        ...
                        <span className="truncate text-sm" title={broker.license}>{broker.license}</span>
                    </div>
                </div>
                <div className="space-y-1.5 min-w-0">
                    <p className="text-muted-foreground text-xs uppercase tracking-wider font-semibold truncate">Min Deposit</p>
                    <p className="font-bold text-accent text-lg font-mono tracking-tight truncate">{broker.minDep}</p>
                </div>
                <div className="space-y-1.5 min-w-0">
                    <p className="text-muted-foreground text-xs uppercase tracking-wider font-semibold truncate">Đòn bẩy</p>
                    <p className="font-bold text-foreground dark:text-gray-200 font-mono text-base tracking-tight truncate">{broker.maxLev}</p>
                </div>
                <div className="col-span-2 md:col-span-1 space-y-2 min-w-0">
                    <p className="text-muted-foreground text-xs uppercase tracking-wider font-semibold truncate">Ưu điểm</p>
                    <div className="flex flex-col gap-2 min-w-0">
                        {broker.features.slice(0, 2).map((feature: string, i: number) => (
                            <span key={i} className="flex items-start gap-2 text-muted-foreground text-sm font-medium leading-tight overflow-hidden">
