...
export default function SystemStatusPage() {
    ...
    async function handleExportBackup() {
        ...
        try {
            ...
            a.href = url;
            a.download = `sanuytin-cms-backup-${dateStr}.json`;
    ...
    async function runChecks() {
        ...
        // 4. Auto-Publish Check (direct DB check instead of API)
        try {
            const now = new Date().toISOString();
            ...
            if (scheduledPosts && scheduledPosts.length > 0) {
                ...
                results.push({ name: 'Auto-Publish', status: 'ok', message: `Đã xuất bản ${scheduledPosts.length} bài`, icon: <Clock size={18} />, details: scheduledPosts.map(p => p.title).join(', ') });
            } else {
                results.push({ name: 'Auto-Publish', status: 'ok', message: 'Không có bài nào cần xuất bản', icon: <Clock size={18} />, details: 'Tất cả bài lên lịch chưa đến giờ' });
            }
        } catch (e) {
            results.push({ name: 'Auto-Publish', status: 'error', message: 'Lỗi kiểm tra', icon: <Clock size={18} />, details: String(e) });
        }
        ...
        // 6. Server
        results.push({ name: 'Next.js Server', status: 'ok', message: 'v14 — Đang chạy', icon: <Globe size={18} />, details: 'Next.js 14 App Router' });
        ...
        <div className="space-y-6 max-w-5xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Kiểm tra kết nối, dữ liệu và dịch vụ</p>
                </div>
                <button onClick={runChecks} disabled={loading}
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm disabled:opacity-50 transition-all hover:shadow-lg hover:shadow-orange-500/25 w-fit">
            ...
            {/* Overall Banner */}
            {!loading && (
                <div className={`rounded-2xl border p-5 flex items-center gap-4 ${overallStatus === 'ok' ? 'bg-emerald-50 border-emerald-200' : overallStatus === 'warning' ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200'}`}>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <h2 className="font-semibold text-gray-900 flex items-center gap-2"><Activity size={16} className="text-orange-400" /> Kiểm tra dịch vụ</h2>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100">
                    <h2 className="font-semibold text-gray-900 flex items-center gap-2"><Database size={16} className="text-orange-400" /> Sao lưu &amp; Khôi phục</h2>
                </div>
                <div className="p-5 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="border border-emerald-200 bg-emerald-50/50 rounded-xl p-5">
                            ...
                            <button onClick={handleExportBackup} disabled={exporting}
                                className="w-full px-4 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-medium hover:bg-emerald-700 disabled:opacity-50 transition-all flex items-center justify-center gap-2">
                        ...
                        <div className="border border-blue-200 bg-blue-50/50 rounded-xl p-5">
                            ...
                            <label className="w-full px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-all flex items-center justify-center gap-2 cursor-pointer">
                    ...
                    {importResult && (
                        <div className="bg-gray-900 rounded-xl p-4 text-xs text-gray-300 font-mono whitespace-pre-line">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                    {[
                        { label: 'CMS', value: 'Sàn Uy Tín CMS v1.0' },
            ...
            <div className="text-center py-4">
                <p className="text-xs text-gray-300">Powered by <span className="font-semibold text-orange-400">Sàn Uy Tín</span> &copy; {new Date().getFullYear()}</p>
            </div>
        </div>
    );
}
