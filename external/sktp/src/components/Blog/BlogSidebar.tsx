...
export default function BlogSidebar({ recentPosts, categories, toc }: BlogSidebarProps) {
    return (
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            ...
            {toc && toc.length > 0 && (
                <div style={{ background: 'white', padding: '24px', borderRadius: '24px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                    <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-heading)', marginBottom: '16px', fontFamily: 'var(--font-display)' }}>Nội dung bài viết</h3>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {toc.map((item, index) => (
                            <li key={index} style={{ paddingLeft: item.level === 3 ? '16px' : '0' }}>
                                <a
                                    href={`#${item.id}`}
                                    style={{
                                        color: 'var(--text)',
