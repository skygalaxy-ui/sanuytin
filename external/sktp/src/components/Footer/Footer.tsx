...
export default async function Footer({ content: propContent }: FooterProps) {
    let content = propContent;
    if (!content) {
        const settings = await getSiteSettings();
        const rawContent = settings.page_content || {};
    ...
    const cDesc = content?.footer_description || 'Đơn vị tổ chức sự kiện uy tín chuyên nghiệp hàng đầu tại Việt Nam.';
    const cHotline = content?.footer_hotline || 'Hotline: 0854 517 868';
    const cEmail = content?.footer_email || 'info@sukientoanquoc.com';
    const cCopyright = content?.footer_copyright || '© 2026 Sự Kiện Toàn Quốc National Events. All Rights Reserved.';
    const fb = content?.footer_facebook || 'https://www.facebook.com/sukientoanquoc';
    const ig = content?.footer_instagram || '#';
    const yt = content?.footer_youtube || '#';
    const tt = content?.footer_tiktok || '#';
    return (
                ...
                <div className={styles.top}>
                    ...
                    <div className={styles.columns}>
                        <div className={styles.col}>
                            ...
                            <Link href="/dich-vu/to-chuc-teambuilding">Teambuilding</Link>
                            <Link href="/dich-vu/to-chuc-company-trip">Company Trip</Link>
                            <Link href="/dich-vu/to-chuc-year-end-party">Year End Party</Link>
                            <Link href="/dich-vu/to-chuc-workshop">Workshop & Đào tạo</Link>
                        </div>
                        <div className={styles.col}>
                            <h4 className={styles.colTitle}>Pháp lý & Tra cứu</h4>
